...
public class WailsBridge {
    ...
    private static native void nativeHandleSpotifyCallback(String code);
    private static native void nativeHandleSpotifyError(String error);
    ...
    /**
     * Handle Spotify OAuth callback from the custom scheme redirect.
     */
    public void handleSpotifyCallback(final String code) {
        if (!initialized) {
            Log.w(TAG, "Bridge not initialized, cannot handle Spotify callback");
            return;
        }
        if (DEBUG) Log.d(TAG, "Spotify callback received with code: " + code.substring(0, Math.min(8, code.length())) + "...");
        try {
            nativeHandleSpotifyCallback(code);
        } catch (Exception e) {
            Log.e(TAG, "Error handling Spotify callback", e);
        }
    }

    /**
     * Handle Spotify OAuth error from the custom scheme redirect.
     */
    public void handleSpotifyError(final String error) {
        if (!initialized) {
            Log.w(TAG, "Bridge not initialized, cannot handle Spotify error");
            return;
        }
        if (DEBUG) Log.d(TAG, "Spotify OAuth error: " + error);
        try {
            nativeHandleSpotifyError(error);
        } catch (Exception e) {
            Log.e(TAG, "Error handling Spotify error", e);
        }
    }
    ...
    /**
     * Open a URL in a Chrome Custom Tab so OAuth can hand control back to the
     * app (the custom tab closes automatically when the redirect deep-links
     * into Jota) instead of leaving a browser tab behind.
     */
    public void openURL(final String url) {
        mainHandler.post(() -> {
            try {
                ...
                Log.e(TAG, "CustomTabs unavailable, falling back to browser", e);
                try {
                    Intent view = new Intent(Intent.ACTION_VIEW, Uri.parse(url));
                    view.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    activity.startActivity(view);
                } catch (Exception e2) {
                    Log.e(TAG, "openURL failed", e2);
                }
            }
        });
    }
