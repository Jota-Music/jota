...
public class MainActivity extends AppCompatActivity {
                        ...
                        if (PermissionRequest.RESOURCE_VIDEO_CAPTURE.equals(resource)) {
                            if (checkSelfPermission(Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                                ...
                                requestPermissions(
                                        ...
                                        WEBVIEW_CAMERA_PERMISSION_REQUEST);
                            }
                            return;
                        }
                    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (data != null && "jota".equals(data.getScheme()) && "callback".equals(data.getHost())) {
            if (DEBUG) Log.d(TAG, "Received OAuth callback: " + data.toString());
            String code = data.getQueryParameter("code");
            String error = data.getQueryParameter("error");
            if (code != null && bridge != null) {
                bridge.handleSpotifyCallback(code);
            } else if (error != null && bridge != null) {
                bridge.handleSpotifyError(error);
            }
            // This intent is just the "back to the app" signal: the auth code
            // already reached the Go callback server via the loopback redirect.
            // Do NOT reload the WebView here - it would tear down the pending
            // login promise in the frontend and the app would never notice the
            // login has completed.
            if (DEBUG) Log.d(TAG, "OAuth return; resuming without WebView reload");
        }
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == WEBVIEW_CAMERA_PERMISSION_REQUEST) {
            if (pendingWebCameraRequest != null) {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    pendingWebCameraRequest.grant(new String[]{PermissionRequest.RESOURCE_VIDEO_CAPTURE});
                } else {
